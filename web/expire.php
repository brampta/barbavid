...
function logstuff($logmessage){
	$expire_log='/var/www/barbavid/expire_log.log';
	if(!file_exists($expire_log)){
		file_put_contents($expire_log,'<script type="text/javascript" src="/maketimus.js"></script>');
	}
...
<script type="text/javascript" src="/language_maketimus_js.php?lang=fr"></script>
</head>
<body>';
...
foreach($ingest_index as $key=>$value)
{
    ...
    {
        //read dat
        $file = fopen("/home/goblet/barbavid/dat_system/".$value.".dat", "r") or exit("Unable to open file!");
        flock($file, LOCK_SH) or exit("Unable to lock file!");
        while(!feof($file))
        {
            $line_data=trim(fgets($file));
