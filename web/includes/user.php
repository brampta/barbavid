...
class User{
        ...
        if($password!=$password2){
            ...
            if ($already_registered) {
                ...
                if (!$id) {
                    ...
                    ob_start();
                    include(dirname(dirname(__FILE__)) . '/user/email/email_validation.php');
    ...
    public function send_password_reset_email($email){
        ...
        if($user_data_array){
            ...
            ob_start();
            include(dirname(dirname(__FILE__)) . '/user/email/reset_password.php');
            $email_body = ob_get_contents();
            ob_end_clean();
            $subject = 'email validation';
            include(dirname(__FILE__) . '/mail.php');
    ...
    public function autologin(){
        ...
        if(isset($_COOKIE['remember_me_token'])){
            ...
            if(!$remember_me_code_data_array){
                ...
                $return_data['success']=true;
                $_SESSION['user_id']=$remember_me_code_data_array['id'];
                $_SESSION['name']=$user_data_array['name'];

                //rotate token
                $token = bin2hex(random_bytes(64));
                setcookie("remember_me_token", $token, time()+31536000, '/', '.'.$main_domain);
                $db->delete_by('remember_me_codes','user_id',$remember_me_code_data_array['id']);
                $remember_me_code_data_array=array();
                $remember_me_code_data_array['user_id']=$remember_me_code_data_array['id'];
                $remember_me_code_data_array['code']=$token;
                $db->insert('remember_me_codes',$remember_me_code_data_array);
            }
        }
